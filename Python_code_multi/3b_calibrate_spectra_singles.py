...
INDIVIDUAL_SAVE_LOCATION = (
    PATH3 + f"Processed_Data_soph_single/prepared_individual_ints/"
)
Path(INDIVIDUAL_SAVE_LOCATION).mkdir(parents=True, exist_ok=True)
INDIVIDUAL_SPECTRUM_SAVE_LOCATION = (
    PATH3 + f"PROCESSED_EXAMPLE/Processed_Data/individual_spectra/"
)
...
for FOLDER in FOLDERS_EXAMINING:  # folders 2, 3 are 50º; folders 4, 5 are 130º; change numbers to whatever folder is desired
    ...
    for i, name in enumerate(int_list):
        ...
        inter_temp, times_temp, angle_temp = cal.load_single_int(
            name
        )  # using the function for averaged interferograms, but passing a single (not averaged) interferogram into it
...
for FOLDER in FOLDERS_EXAMINING:  # folders 2, 3 are 50º; folders 4, 5 are 130º; change numbers to whatever folder is desired
    ...
    int_list.sort()
    total_ints = len(int_list)
    Path(INDIVIDUAL_SPECTRUM_SAVE_LOCATION + FOLDER[len(INT_LOCATION) :]).mkdir(
