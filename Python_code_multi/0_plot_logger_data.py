...
pth_time, pth_pressure, pth_temp, pth_humidity = cal.load_pth(PTH_DATA_LOCATION)
...
axs1a.plot(gui_data["time"], gui_data["HBB"], label="Hot BB temperature (C)", color="g")
axs1[0, 0].set(xlim=time_lims, ylabel="Hot BB temperature (C)")
...
# )
axs1[0, 1].set(xlim=time_lims, ylabel="Cold BB temperature (C)")
# axs1b.set_ylabel("Cold BB temperature (C)", color="g")
# axs1b.set_ylim([31, 31.5])
# axs1b.tick_params(axis="y", labelcolor="g")

axs1[1, 0].plot(
    gui_data["time"], gui_data["PRT1"], label="PRT1 - Front of electronics box"
)
