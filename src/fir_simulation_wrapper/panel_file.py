...
class panel_file_hdr:
    def __init__(self, _f, data_types):
        unpack_fmt = "=" + data_types["junk"] + "80sd" + 2 * data_types["float"]
    ...
    def __repr__(self):
        ...
        for item in self.lblrtm_flag.items():
            lblrtm_flag_str = lblrtm_flag_str + f"    {item[0]:12s}: {item[1]:g}\n"
        ...
        return (
            ...
            + "{0:12s}: {1:f}\n".format("yi1", self.yi1)
            + yid_fmt_str.format(
                "yid", self.yid[0], self.yid[1], self.yid[2], self.yid[3], self.yid[4]
            )
        )
...
class panel_file:
    ...
    def __init__(
        ...
        self._data_types = {"junk": junk_type, "int": int_type, "float": float_type}
    ...
    def load_data(self):
        ...
        panel_dtype = np.dtype(
            [("v1", "float64"), ("v2", "float64"), ("dv", "float64"), ("n_pts", "int")]
        )
        self.panel_hdrs = np.zeros(self.n_panels, panel_dtype)
        self.data1 = np.zeros(self.n_pts, "float32")
        if self.panel_format == "double":
            self.data2 = np.zeros(self.n_pts, "float32")
        ...
        for n in range(self.n_panels):
            ...
            panel_data = self._fread_panel(self.panel_format, panel_hdr[3], "read")
        ...
        self.v = np.linspace(self.vmin, self.vmax, self.n_pts)
    ...
    def _check_field_type(self):
        ...
        raise OSError("Failed to determine field size in LBLRTM Panel file")
    ...
    def _check_panel_format(self):
        ...
        if not self._valid_panel_hdr(panel_hdr):
            raise OSError("Panel header not successfully read")
        ...
        if single_panel_valid:
            ...
            self._f.close()
            raise OSError("Could not determine panel format")
    ...
    def _check_length(self):
        ...
        while not at_last_panel and not at_EOF:
            ...
            if n_pts_in_panel > 0:
                ...
                panel_size = self._fread_panel(
                    self.panel_format, n_pts_in_panel, "seek"
                )
            else:
                at_last_panel = True
    ...
    def _valid_header_bytes(self, test_bytes, test_junk_type, test_float_type):
        unpack_fmt = "=" + test_junk_type + "80sd" + 2 * test_float_type
        test_data = struct.unpack(
            unpack_fmt, test_bytes[0 : struct.calcsize(unpack_fmt)]
        )
        return (
    ...
    def _fread_panel_hdr(self):
        ...
        unpack_fmt = (
            ...
            + "dd"
            + self._data_types["float"]
            + self._data_types["int"]
        )
    ...
    def _fread_panel(self, panel_type, panel_len, read_file):
        ...
        if read_file == "no":
            ...
            if panel_type == "single":
                ...
                data = [
                    data[1 : panel_len + 1],
                    data[panel_len + 3 : 2 * panel_len + 3],
                ]
            return data
